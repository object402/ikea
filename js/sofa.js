$(document).ready(function() {
   ...
   $('.j_sofaList ul li').on('mouseover',function(){
      var index = $(this).index()+1;
      $(this).find('img').fadeTo('slow').css({'width':250}).attr('src','img/sofa/'+index+'1.JPG');
   }).on('mouseleave',function(){
      var index = $(this).index()+1;
      $(this).find('img').css({'width':250}).attr('src','img/sofa/'+index+'0.JPG');
   })
...
//list 정렬 Test
   var listLength = $('.j_sofaList ul li').length;
   //console.log(listLength);
   var nameArray = new Array();
   for(var i = 0; i <listLength; i++ ){
      nameArray[i] = $('.j_sofaList ul li:eq(' + i + ') a div:last-child p:first-child').text();
   }

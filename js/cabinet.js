$(document).ready(function() {
   ...
   $('.j_storageList ul li').on('mouseover',function(){
      var index = $(this).index()+1;
      $(this).find('img').fadeTo('slow').css({'width':250}).attr('src','img/cabinet/'+index+'1.JPG');
   }).on('mouseleave',function(){
      var index = $(this).index()+1;
      $(this).find('img').css({'width':250}).attr('src','img/cabinet/'+index+'0.JPG');
   })
...
//list 정렬 Test
   var listLength = $('.j_storageList ul li').length;
   //console.log(listLength);
   var nameArray = new Array();
   for(var i = 0; i <listLength; i++ ){
      nameArray[i] = $('.j_storageList ul li:eq(' + i + ') a div:last-child p:first-child').text();
   }
